...
DLIS_INPUT_DIR = os.path.join(BASE_PATH, "DLIS_input/SC")
...
def format_simple(name):
    ...
    # Padrão 2: Nomes com 4 letras (1RCH_0001__SC)
        ...
        parts = name.split('_')
        num_letras = parts[0]          # '1RCH'
        numero = parts[1]              # '0001'
    
    # Extrai dígitos e letras iniciais
    prefix_num = ''.join([c for c in num_letras if c.isdigit()])  # '1'
    prefix_let = ''.join([c for c in num_letras if c.isalpha()])  # 'MB' ou 'RCH'
    
    return f"{prefix_num}_{prefix_let}_{int(numero)}_SC"  # Padrão final: 1_MB_1_SC ou 1_RCH_1_SC
